  ...
  postprocess: function(messages) {
    ...
    if (state.frontMatter) {
      ...
      for (i = 0; i < flattenedMessages.length; ++i) {
        ...
        if ('fix' in flattenedMessages[i]) {
          flattenedMessages[i].fix.range[0] += frontMatterChars;
          flattenedMessages[i].fix.range[1] += frontMatterChars;
        }
      }
    }
